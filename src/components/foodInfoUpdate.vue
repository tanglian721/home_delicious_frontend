<template>
  <div class="food-info-upload">
    ...
    <div v-else class="edit-div">
      ...
      <div class="other-info">
        <div class="cooking_way info">
          <span><span v-if="this.$store.getters.lan" >烹饪方式：</span><span v-else >Cooking method： </span></span>
          <select id="cook" name="cook" @change="Cook">
            <option class="option" value="fry"><span v-if="this.$store.getters.lan" >炒</span><span v-else >fry</span></option>
            <option class="option" value="deep fry"><span v-if="this.$store.getters.lan" >炸</span><span v-else >deep fry</span></option>
            <option class="option" value="stew"><span v-if="this.$store.getters.lan" >烧</span><span v-else >stew</span></option>
            <option class="option" value="steam"><span v-if="this.$store.getters.lan" >蒸</span><span v-else >steam</span></option>
            <option class="option" value="bake"><span v-if="this.$store.getters.lan" >烤</span><span v-else >bake</span></option>
            <option class="option" value="salad"><span v-if="this.$store.getters.lan" >拌</span><span v-else >salad</span></option>
          </select>
        </div>
        <div class="info">
          <span><span v-if="this.$store.getters.lan" >难度：</span><span v-else >difficulty：</span> </span>
          <select id="difficult" name="difficult" @change="Difficulty">
            <option class="option" value="easy"><span v-if="this.$store.getters.lan" >简单</span><span v-else >easy</span></option>
            <option class="option" value="medium"><span v-if="this.$store.getters.lan" >中等</span><span v-else >medium</span></option>
            <option class="option" value="difficult"><span v-if="this.$store.getters.lan" >困难</span><span v-else >difficult</span></option>
            <option class="option" value="super"><span v-if="this.$store.getters.lan" >大师</span><span v-else >super</span></option>
          </select>
        </div>
        <div class="info">
          <span><span v-if="this.$store.getters.lan" >烹饪时间：</span><span v-else >Cook Time:</span></span>
          <span>
            <input type="text" v-model="time" placeholder="30" /><span v-if="this.$store.getters.lan" >分钟</span><span v-else >Min</span>
          </span>
        </div>
        <div class="info">
          <span><span v-if="this.$store.getters.lan" >地域：</span><span v-else >Location:</span></span>
          <input type="text" v-model="location" placeholder="China" />
        </div>
        <div class="info">
          <span><span v-if="this.$store.getters.lan" >类别：</span><span v-else >Category:</span></span>
          <input type="text" v-model="category" placeholder="pasta" />
        </div>
        <div class="info">
          <span><span v-if="this.$store.getters.lan" >标签：</span><span v-else >Tag:</span></span>
    ...
    <div class="button">
      <button v-if="ifpreview" @click="backEdit"><span v-if="this.$store.getters.lan" >继续编辑</span><span v-else >Continue Edit</span></button>
      <button v-else @click="preview"><span v-if="this.$store.getters.lan" >预览</span><span v-else >preview</span></button>
      <button v-if="ifpreview" @click="updateFood"><span v-if="this.$store.getters.lan" >确认修改</span><span v-else >update</span></button>
      <button v-else @click="method"><span v-if="this.$store.getters.lan" >修改制作方法</span><span v-else >Edit Method</span></button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    updateFood() {
      console.log(this.editFood.food_id,);
      axios
        .request({
  ...
  watch: {
    ...
    editFood(newValue) {
      ...
      document.getElementById("difficult").value = this.difficulty;
      this.cook_way = document.getElementById("cook").value = this.cook_way
    },
  },
  computed: {
    ifupload() {
