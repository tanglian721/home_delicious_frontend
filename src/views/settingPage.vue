<template>
  <div class="setting-page">
    ...
    <div class="setting-option">
      <h2><span v-if="this.$store.getters.lan" >设置：</span><span v-else >Setting：</span></h2>
      <button @click="editUser" ><span v-if="this.$store.getters.lan" >修改用户信息</span><span v-else >Modify Account</span></button>
      <button @click="DeleteUser" ><span v-if="this.$store.getters.lan">删除用户</span><span v-else >Delete Account</span></button>
      <button @click="pop_up = !pop_up" ><span v-if="this.$store.getters.lan">更改语言</span><span v-else >Language Setting</span></button>
    </div>
